...
- **Perfil persistido no navegador** (localStorage `esquentaProfile`): carregado com merge defensivo na inicialização; `saveProfile()` deve ser chamado em toda mutação de `localProfile`. A chave `esquentaPerfilPronto` marca quem já concluiu a personalização — essa pessoa entra direto na pista (`enterParty()` no fim do script), sem o modal.
...
- **Ações do líder** (faxina da fila, DJ ausente, limite de 10 min, avanço no fim) rodam só no `isLeader()`, que agora elege entre os **presentes** — aba escondida tem timer estrangulado e não serve como líder.

## Firestore (caminho base `artifacts/{appId}/public/data/…`)

| Coleção/Doc | Uso |
|---|---|
| `users/{uid}` | presença + perfil (x, y, avatarConfig, color, isVIP, lastUpdate, away, awaySince) |
